...
struct a_vm *a_vm_init(struct a_vm *self) {
  ...
  a_ls_insert(&self->scopes, &self->main.vm_scopes);
...
struct a_op *a_vm_emit(struct a_vm *self, enum a_op_type op_type) {
  ...
  a_ls_insert(&self->code, &op->vm_code);
  return op;
}
...
struct a_val *a_vm_pop(struct a_vm *self) {
  return a_ls_null(&self->stack) ? NULL : a_baseof(a_ls_remove(self->stack.prev), struct a_val, vm_stack);
}
